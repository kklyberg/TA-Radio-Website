...
function renderProduct(product) {
    ...
    // ✅ FIXED: Sanitizes the incoming database image string to strip out query parameters
    const mainImgElement = document.getElementById('primaryProductImg');
    if (mainImgElement) {
        let rawLink = product.image || '';
        
        if (rawLink) {
            // Strip out any dynamic image sizing arguments like &height=300 or ?width=300
            if (rawLink.includes('&')) rawLink = rawLink.split('&')[0];
            if (rawLink.includes('?')) rawLink = rawLink.split('?')[0];
            
            // Clean up html entity encodings (&amp;) just in case they slipped into the sheet
            rawLink = rawLink.replace(/&amp;/g, '&');
        }
        
        // Load the clean image path, or fall back to your radio placeholder graphic if empty
        mainImgElement.src = rawLink || 'images/radio-placeholder.png';
    }

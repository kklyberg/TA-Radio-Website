...
const lossThresholds = {
    "open": { lossFactor: 2.0, clutterLoss: 0, text: "Open Flat Plains: Signal follows absolute Line-of-Sight geometric rules. Range is limited primarily by node elevation heights and the physical curvature of the Earth." },
    "suburban": { lossFactor: 2.7, clutterLoss: 12, text: "Suburban Environment: Light residential clutter introduces multipath fading, degrading active signal thresholds by roughly 12dB." },
    "foliage": { lossFactor: 3.4, clutterLoss: 24, text: "Dense Forest Canopy: Heavy wood foliage absorption locks onto radio wavelengths. VHF performs best here, while UHF attenuates rapidly." },
    "urban": { lossFactor: 4.2, clutterLoss: 38, text: "Dense Urban Grid: Steel frames and concrete walls aggressively block, drop, and bounce signal waves. High-elevation infrastructure is mandatory." }
};
...
function processTelemetryAndData() {
    ...
    const txHeight = parseFloat(document.getElementById("hudTxHeight").value);
    const rxHeight = parseFloat(document.getElementById("hudRxHeight").value);
    const rawPower = parseFloat(document.getElementById("rfPower").value);
    ...
    if (useRepeater) {
        if (repDeck) repDeck.style.display = "flex"; // Forces panel visibility to unfold cleanly
        
        const repHeight = parseFloat(document.getElementById("hudRepHeight").value);
        const repPower = parseFloat(document.getElementById("rfRepPower").value);
        ...
        // TRUE REPEATER MATHEMATICS: Leg A (Handheld to Tower) + Leg B (Tower to Base Station)
        const horizonTxToTower = Math.sqrt(2 * txHeight) + Math.sqrt(2 * repHeight);
        const horizonTowerToRx = Math.sqrt(2 * repHeight) + Math.sqrt(2 * rxHeight);
        maxGeometricLOS = horizonTxToTower + horizonTowerToRx;
        calculatedUsableRange = maxGeometricLOS * (1 - (lossThresholds[terrainKey].lossFactor * 0.08));
    } else {
        if (repDeck) repDeck.style.display = "none"; // Safe container hiding loop collapse
        
        maxGeometricLOS = Math.sqrt(2 * txHeight) + Math.sqrt(2 * rxHeight);
        let baseCoverage = maxGeometricLOS * 0.45;
        if (terrainKey === "urban") baseCoverage *= 0.18;
        if (terrainKey === "foliage") baseCoverage *= 0.35;
        if (terrainKey === "suburban") baseCoverage *= 0.60;
        calculatedUsableRange = Math.max(0.4, baseCoverage);
    }
